...
class OrderSynchronizer:
    ...
    def _get_order_status_from_history(self, order_id: str) -> Dict:
        """Récupère le statut d'un Order ID avec priorité aux ordres ouverts
        
        LOGIQUE CORRIGÉE:
        1. Si l'ordre est dans open_orders -> TOUJOURS retourner 'open' (priorité absolue)
        2. Sinon, chercher dans user_fills -> retourner 'filled' avec la quantité totale
        3. Sinon -> 'unknown'
        ...
        """
        try:
            # 1. ✅ PRIORITÉ ABSOLUE : Vérifier si l'ordre est OUVERT
            ...
            for order in open_orders:
                if str(order.get('id')) == str(order_id):
                    # ✅ ORDRE OUVERT - Retour immédiat, peu importe l'historique
                    self.logger.info(f"📊 Order ID {order_id} - Status: OPEN (priorité)")
                    return {
                        'status': 'open',
            ...
            # 2. L'ordre n'est PAS ouvert, vérifier s'il a été rempli
            try:
            ...
            # Calculer la quantité totale remplie
            total_filled = 0.0
            latest_fill_time = 0
            ...
            if total_filled > 0:
                # ✅ Ordre rempli (totalement ou partiellement, mais plus dans open_orders)
